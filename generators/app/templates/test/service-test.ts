...
import { <%= capitalizedServiceName %>Service } from '../src/service';
import accountConfig from './fake-account-config';

describe('<%= lowerServiceName %> service deployer', () => {
    ...
    const serviceType = '<%= lowerServiceName %>';
    const serviceProvisioner = new <%= capitalizedServiceName %>Service();

    beforeEach(async () => {
        serviceParams = {
            type: serviceType
        };
        serviceContext = new ServiceContext(appName, envName, serviceName, new ServiceType('<%= lowerExtensionName %>', serviceType), serviceParams, accountConfig);
